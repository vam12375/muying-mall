...
public class AdminCategoryController {
    ...
    public Result<List<Category>> getAllCategoriesFlat() {
        try {
    ...
    public Result<Boolean> createCategory(@RequestBody Category category) {
        try {
            ...
            if (result) {
                return Result.success(true, "创建分类成功");
            } else {
    ...
    public Result<Boolean> updateCategory(@PathVariable Integer id, @RequestBody Category category) {
        try {
            category.setCategoryId(id);
            boolean result = categoryService.updateById(category);
            if (result) {
                return Result.success(true, "更新分类成功");
            } else {
    ...
    public Result<Boolean> updateCategoryStatus(@PathVariable Integer id, @RequestParam Integer status) {
        try {
            // 检查状态值是否合法
            if (status != 0 && status != 1) {
                return Result.error("状态值不合法，只能为0或1");
            }
            
            boolean result = categoryService.updateStatus(id, status);
            if (result) {
                return Result.success(true, status == 1 ? "启用分类成功" : "禁用分类成功");
            } else {
    ...
    public Result<Integer> getCategoryProductCount(@PathVariable Integer id) {
        try {
